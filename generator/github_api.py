...
class GitHubAPI:
    ...
    def _request(self, method: str, url: str, **kwargs) -> requests.Response:
        ...
        # If rate-limited without token, raise so caller uses fallback defaults
        if resp.status_code == 403 and "rate limit" in resp.text.lower():
            logger.warning("Rate limited by GitHub API. Using fallback stats.")
            raise requests.exceptions.RequestException("GitHub API Rate Limited")
